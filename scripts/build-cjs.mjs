...
const exportedNames = Array.from(source.matchAll(/^export (?:async )?function ([a-zA-Z0-9_$]+)/gm), (m) => m[1]);
...
let cjs = source
  ...
  .replace("import { createCipheriv, createDecipheriv, createHash, randomUUID, timingSafeEqual } from 'node:crypto';", "const { createCipheriv, createDecipheriv, createHash, randomUUID, timingSafeEqual } = require('node:crypto');")
